...
export function validateOverload(context: ValidationContext): Violation[] {
  ...
  for (const semester of semesters) {
    ...
    if (totalUnits > MAX_UNITS_PER_SEMESTER) {
      ...
      violations.push({
        ...
        semesterLabel,
        message: `${semesterLabel} has ${totalUnits} units (max allowed: ${MAX_UNITS_PER_SEMESTER})`,
        suggestion: `Remove ${totalUnits - MAX_UNITS_PER_SEMESTER} units from this semester`,
      })
